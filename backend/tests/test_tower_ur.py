...
def test_three_scans_within_window_start_holding(db: Session):
  scenario_id, zone_id, points = _make_scenario_with_zone(db)
  now = datetime.utcnow()

  zone = scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[0], now=now)
  assert zone.status == "syncing"
  zone = scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[1], now=now + timedelta(seconds=60))
  assert zone.status == "syncing"
  zone = scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[2], now=now + timedelta(seconds=120))
...
def test_scan_outside_window_resets_to_idle(db: Session):
  ...
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[0], now=now)
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[1], now=now + timedelta(seconds=60))
  # третья точка отсканирована слишком поздно — окно от первого скана истекло
  zone = scan_sbg_ur_point(
    db, scenario_id=scenario_id, point=points[2], now=now + timedelta(seconds=300)
  )
...
def test_hold_completes_after_timer(db: Session):
  ...
  now = datetime.utcnow()
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[0], now=now)
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[1], now=now + timedelta(seconds=30))
  zone = scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[2], now=now + timedelta(seconds=60))
  assert zone.status == "holding"
...
def test_drg_scan_resets_during_holding(db: Session):
  ...
  now = datetime.utcnow()
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[0], now=now)
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[1], now=now + timedelta(seconds=10))
  zone = scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[2], now=now + timedelta(seconds=20))
  assert zone.status == "holding"
...
def test_drg_scan_after_capture_is_noop(db: Session):
  ...
  now = datetime.utcnow()
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[0], now=now)
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[1], now=now)
  scan_sbg_ur_point(db, scenario_id=scenario_id, point=points[2], now=now)
