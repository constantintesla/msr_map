"""CRUD геометрии объектов карты (только при idle)."""
...
def _next_kt_number(db: Session) -> int:
  points = db.query(Point).filter(Point.stage == 1).all()
  nums = [n for p in points if (n := kt_number_from_name(p.name)) is not None]
  return max(nums, default=0) + 1


def _next_post_number(db: Session) -> int:
  points = db.query(Point).filter(Point.stage == 3).all()
  nums: list[int] = []
  for p in points:
...
def _next_film_loot_number(db: Session) -> int:
  caches = db.query(Cache).filter(Cache.stage == 2, Cache.cache_kind == "film_loot").all()
  nums: list[int] = []
  for c in caches:
...
def _next_mertvyak_number(db: Session, team_side: str) -> int:
  caches = (
    db.query(Cache)
    .filter(Cache.stage == 3, Cache.cache_kind == "mertvyak", Cache.team_side == team_side)
    .all()
  )
  return len(caches) + 1
...
def create_point(
  ...
  *,
  ...
  if name is None:
    name = f"КТ{_next_kt_number(db)}" if stage == 1 else f"Пост-{_next_post_number(db)}"
...
def create_cache(
  ...
  *,
  ...
  if name is None:
    if cache_kind == "film_loot":
      film_idx = _next_film_loot_number(db)
      name = f"Ящик {film_idx}"
    elif cache_kind == "mertvyak":
      side_label = "ЛК" if team_side == "A" else "СБГ"
      name = f"Мертвяк {side_label} {_next_mertvyak_number(db, team_side or 'A')}"
    else:
...
def wipe_all_objects(db: Session) -> tuple[int, int, int]:
  """Полностью удалить все точки, схроны и ориентиры (только до старта игры)."""
  assert_game_idle(db)

  n_points = db.query(Point).count()
  n_caches = db.query(Cache).count()
  n_landmarks = db.query(Landmark).count()
